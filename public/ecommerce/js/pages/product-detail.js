(function () {
    ...
    async function postCartJSON(payload, btn) {
        ...
        try {
            if (btn) { btn.classList.add('disabled'); btn.setAttribute('aria-disabled', 'true'); btn.style.pointerEvents = 'none'; }
            const res = await fetch(url, { method: 'POST', credentials: 'same-origin', headers, body: JSON.stringify(payload) });
            if (res.status === 401) {
                window.location.href = '/login-register';
                return;
            }
            if (!res.ok) {
                const text = await res.text(); throw new Error(`Gagal (${res.status}). ${text.slice(0, 200)}`);
            }
            ...
            if (btn) { btn.classList.remove('disabled'); btn.setAttribute('aria-disabled', 'false'); btn.style.pointerEvents = ''; }
        }
    }
